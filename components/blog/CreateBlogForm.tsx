...
useEffect(()=>{
  if(uploadedCover){
    setValue("coverImage",uploadedCover,{shouldValidate:true,shouldDirty:true,shouldTouch:true});
  }
},[uploadedCover])

useEffect(()=>{
  if(typeof content === 'string'){
    setValue("content",content,{shouldValidate:true,shouldDirty:true,shouldTouch:true});
  }
},[content])
...
createBlog({...data,isPublished:false}).then((data)=>{
  ...
  return (
    <form className="flex flex-col justify-between max-w-[1200px] m-auto min-h-[85vh]"  onSubmit={handleSubmit(onPublish)}>
      ...
      <div>
        ...
        <FormField id="title" register={register} errors={errors} placeholder="Blog Title " disabled={false} inputClassName="border-none text-5xl font-bold bg-transparent px-0"></FormField>
        <fieldset className="flex flex-col border-y mb-4 py-2">
          <legend className="mb-2 pr-2">
            Select 4 Tags
        ...
        <BlockNoteEditor onChange={onChange} initialContent={blog?.content ? blog.content : ""}></BlockNoteEditor>
      ...
      <div className='border-t pt-2'>
       ...
       <div className="flex items-center justify-between gap-6"> 
        ...
        <div className="flex gap-4">
               ...
               <Button type="button"  label={isSavingAsDraft ? "Saving as draft..." : "Save as draft"} onClick={handleSubmit(onSaveDraft)} />
        </div>
       </div>
      </div>
    </form>
